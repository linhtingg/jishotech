...
if (strlen($_SESSION['uid'] == 0)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['deleteWords']) && is_array($_POST['deleteWords'])) {
            $deleteWords = implode(',', $_POST['deleteWords']);
            ...
            if ($conn->query($deleteQuery)) {
                ...
                echo json_encode(['error' => 'Error deleting words']);
                exit;
            }
        }
    } else {
                    ...
                    <div class="col col-3">
                        ...
                        <div class="topiclist">
                            ...
                            <div class="list-group list-group-custom">
                                <a href="bookmark.php?topic=0" class="list-group-item">全ての単語</a>
                                <?php
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $('#deleteButton').click(function() {
                        var selectedWords = $('input[name="deleteWord[]"]:checked').map(function() {
                            return this.value;
                        }).get();

                        // Send AJAX request to delete selected words in the database
                        $.ajax({
                            type: 'POST',
                            url: '', // Leave it empty to post to the same file
                            data: {
                            ...
                            success: function(response) {
                                try {
                                    ...
                                    if (data.success) {
                                        ...
                                        console.error('Error deleting words:', data.error);
                                    }
                                } catch (error) {
                    ...
                    $('#selectAll').click(function() {
                        $('input[name="deleteWord[]"]').prop('checked', true);
                    });

                    // Handle deselect all button click
                    $('#deselectAll').click(function() {
                        $('input[name="deleteWord[]"]').prop('checked', false);
                    });
